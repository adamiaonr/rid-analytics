...
def main():
    ...
    for line in outcomes_file.readlines():
        ...
        try:
            ...
            if int(splitted[3]) > max_hops:
                max_hops = int(splitted[3])

            _data[penalty_types.index(str(splitted[2]))][fp_prob.index(float(splitted[0]))][alpha.index(float(splitted[1]))][outcomes.index(str(splitted[4]))].append((int(splitted[3]), float(splitted[5])))
    ...
    for i in xrange(len(penalty_types)):
        ...
        for j in xrange(len(fp_prob)):
            ...
            for k in xrange(len(alpha)):
                ...
                for l in xrange(len(outcomes)):
                    ...
                    for m in xrange(max_hops):
                        _data[i][j][k][l].append((m + 1, 0.0))
                ...
                chart.set_xlabel("Nr. of hops", fontsize=LABEL_FONT_SIZE)
        ...
        plt.savefig(sys.argv[3] + "/outcome-vs-hop." + penalty_types[i] + ".cache." + input_file_dir[-1:] + ".png", bbox_inches='tight')
