...
f_distributions[2][0].append([0.0001,0.0001,0.0001,0.0001,0.0001,0.0001,0.0002,0.0003,0.0035,0.0357,0.3695,2.5852,12.297,35.948,48.76])
f_distributions[2][1].append([48.867,35.85,12.286,2.5795,0.3753,0.0385,0.0026,0.0003,0.0002,0.0001,0.0001,0.0001,0.0001,0.0001,0.0001])
            ...
            if file_name.endswith(bf_size + ".csv"):
                ...
                for line in _file.readlines():
                    ...
                    try:
                        ...
                        origin_distance = int(file_name.split("-")[2])
...
#         f_distributions_plot = plt.subplot2grid((5,5), (0,0), rowspan=2, colspan=2)
...
#         # labels
#         if (i > 0):
#             f_distributions_plot.set_xlabel("Prefix length", fontsize=LEGEND_FONT_SIZE)
...
#         f_distributions_plot.legend((_mlocal[0], _m_non_local[0]), ('long prefixes', 'short prefixes'), loc='upper center', fontsize=LEGEND_FONT_SIZE)
...
#     plt.savefig(sys.argv[3] + "/" + sys.argv[4] + "-dist" + ".pdf", format='pdf', bbox_inches='tight')
    ...
    # markers for this graph
    markers = ['-^', '-o', '-d', '-v','-^', '-o', '-d', '-v', '--^', '--o', '--d', '--v']
    markers_size = [7.5, 10, 7.5, 7.5]

    fig = plt.figure()
    outcome_probs = plt.subplot2grid((5,5), (0,0), rowspan=2, colspan=2)
    ...
    for i in xrange(len(sub_dirs)):
        ...
        for j in xrange(len(outcomes)):
            ...
            outcome_probs.plot(x[i][j], y[i][j], markers[(i * 4) +  j], markersize=markers_size[j], linewidth=2, color=colors[j])
            ...
            outcome_probs.set_ylim(1E-25, 5)
            outcome_probs.set_yticks([1E-25, 1E-20, 1E-15, 1E-10, 1E-5, 1E0])

            #outcome_probs.set_yticks([1E-30, 1E-25, 1E-20, 1E-15, 1E-10, 1E-5, 1E0])
        ...
        _r5 = plt.plot([], [], '-', markersize=2, color='black', linewidth=2)
        _r15 = plt.plot([], [], '--', markersize=2, color='black', linewidth=2)

        # if (i == 0):
        #     outcome_probs.legend((_cd[0], _id[0], _fd[0], _r5[0], _r15[0]), (outcomes[0], outcomes[1], outcomes[2], 'R=5', 'R=15'), loc='center left', fontsize=LEGEND_FONT_SIZE, ncol=2)
        # else:
        #     outcome_probs.legend((_cd[0],   _id[0], _fd[0], _r5[0], _r15[0]), (outcomes[0], outcomes[1], outcomes[2], 'R=5', 'R=15'), loc='center left', fontsize=LEGEND_FONT_SIZE, ncol=1)

        if (i == 0):
            outcome_probs.legend((_id[0], _fd[0], _r5[0], _r15[0]), (outcomes[1], outcomes[2], '|R|=5', '|R|=15'), loc='center left', fontsize=LEGEND_FONT_SIZE, ncol=2)
        else:
            outcome_probs.legend((_id[0], _fd[0], _r5[0], _r15[0]), (outcomes[1], outcomes[2], '|R|=5', '|R|=15'), loc='center left', fontsize=LEGEND_FONT_SIZE, ncol=1)


    plt.tight_layout(pad=0.4, w_pad=0.4, h_pad=0.4)
    plt.savefig(sys.argv[3] + "/" + sys.argv[4] + "-outcomes-5-15-uni-fr" + ".pdf", format='pdf', bbox_inches='tight')
    ...
    #     if (i == 0):
    #         outcome_probs.legend((_cd[0], _id[0], _fd[0]), (outcomes[0], outcomes[1], outcomes[2]), loc='center left', fontsize=LEGEND_FONT_SIZE, ncol=1)
    #     else:
    #         outcome_probs.legend((_cd[0], _id[0], _fd[0]), (outcomes[0], outcomes[1], outcomes[2]), loc='lower left', fontsize=LEGEND_FONT_SIZE, ncol=1)
    ...
    #     avg_latencies.bar(x[i], y[i], BAR_WIDTH, color='white', hatch=patterns[i])
