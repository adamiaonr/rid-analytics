...
def main():
    ...
    if len(sys.argv) < 2:
        print "usage: python plot-on-path.py <bf-size> <input-file-dir> <output-file-dir> <output-file-name>"
        return
    ...
    for i in xrange(len(sub_dirs)):
        ...
        for j in xrange(len(outcomes)):
            ...
            outcome_probs.set_xlim(0, 6)
            outcome_probs.set_xticks(np.arange(1, 6, 1))

            outcome_probs.set_ylim(1E-5, 5)
            outcome_probs.set_yticks([1E-5, 1E0])
